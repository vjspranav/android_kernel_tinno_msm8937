...
#define gf_dbg(fmt, args...) do { \
					pr_warn("gf3208:" fmt, ##args);\
		} while (0)
#define FUNC_ENTRY()  pr_warn("gf3208:%s, entry\n", __func__)
#define FUNC_EXIT()  pr_warn("gf3208:%s, exit\n", __func__)
#else
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	FUNC_ENTRY();
	pr_info("%s gf_ioctl,cmd=%d,GFX1XM_IOC_FTM = %d\n", __func__,cmd,(int)GFX1XM_IOC_FTM);
	if (_IOC_TYPE(cmd) != GF_IOC_MAGIC) {
		pr_info("%s IOC_TYPE(cmd) != GF_IOC_MAGIC\n", __func__);
		return -ENODEV;
	}
	...
	case GF_IOC_REQUEST_IRQ: {
		int irq_err = -1;
		printk(KERN_ERR"mingliang.tan  GF_IOC_REQUEST_IRQ\n");
		#if 1
		irq_err = request_threaded_irq(gf_dev->irq, NULL, gf_irq,
		                               IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		                               "gf", gf_dev);
		#else
		irq_err = request_irq(gf_dev->irq, gf_irq,
	...
	case GF_IOC_CHIP_INFO:
		...
		pr_info("chip_type: 0x%x\n", info.chip_type);
		pr_info("so_version: %s\n", info.so_version);
		full_fp_chip_info(info.so_version);
		break;
	default:
		pr_warn("gf: unsupport cmd:0x%x\n", cmd);
		break;
	}
	FUNC_EXIT();
	return retval;
...
#endif
{
	...
	pr_info(" goodix gf3208 gf_probe ========.\n");
	FUNC_ENTRY();
	...
	if (ret) {
		dev_err(&gf_dev->spi->dev, "Failed to power on device\n");
...
static int __init gf_init(void)
{
	...
	if(strstr(saved_command_line,CHARGER_MODE_BOOT)) {
		pr_info("goodix gfx1xm  gf_init  --androidboot.mode=charger  return--- \n");
		return -ENODEV;
	}
	...
	printk("gf3208--platform_driver_register--status=%d\n",status);
	#elif defined(USE_SPI_BUS)
	status = spi_register_driver(&gf_driver);
	printk("gf3208--spi_register_driver--status=%d\n",status);
	#endif
