...
static int tinno_get_hall_is_open(void)   //tinno MBA add   MBA_HDW_Hall_REQC134
{
	char * tinno_sensor_info;

	tinno_sensor_info = tinno_get_sensor_info();
	if(NULL == tinno_sensor_info) {
		printk(KERN_INFO "tinno_get_hall_is_open not MBA, as default open \n");
		return 1;
	}
	printk(KERN_INFO "tinno_get_hall_is_open :tinno_sensor_info = %s \n",tinno_sensor_info);

	if (NULL != strstr(tinno_sensor_info, "H"))
		return 1;
	else
		return 0;
...
static int hall_driver_probe(struct platform_device *dev)
{
	...
	if(!tinno_get_hall_is_open())        //tinno MBA add   MBA_HDW_Hall_REQC134
		return -1;
...
// dong.wang, Date20170227, Modify For Wiko Unify Version, HCABMA-2, Start
#ifdef CONFIG_WIKO_UNIFY
extern int Hall;
#endif  /* CONFIG_WIKO_UNIFY */
// dong.wang, Date20170227, Modify For Wiko Unify Version, HCABMA-2, End

static int __init hall_init(void)
{
// dong.wang, Date20170227, Modify For Wiko Unify Version, HCABMA-2, Start
#ifdef CONFIG_WIKO_UNIFY
	printk(KERN_INFO "Unify hall_init Hall = %d\n", Hall);
	if (!Hall) {
		return 0;
	}
#endif  /* CONFIG_WIKO_UNIFY */
